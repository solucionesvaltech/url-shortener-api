...
var _ = Describe("UpdateHandler", Ordered, func() {
	var (
	...
	When("the param is not present", func() {
		It("should return an error", func() {
			...
			req := httptest.NewRequest(http.MethodPut, fmt.Sprintf("/%s", shortURL), strings.NewReader(requestBody))
	...
	When("the body is invalid", func() {
		It("should return an error", func() {
			req := httptest.NewRequest(http.MethodPut, fmt.Sprintf("/%s", shortURL), nil)
			req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
			ctx = e.NewContext(req, rec)
	...
	When("the use case fails", func() {
		It("should return an error", func() {
			...
			mockUseCase.EXPECT().
				UpdateURL(gomock.Any(), gomock.Any(), gomock.Any()).
